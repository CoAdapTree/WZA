...
def analyseSimSet(simulationSet, PVE_threshold = 0.005, bayPass = "none", IBA = False, shuffle = False, MAF_filter = 0.05, unweighted = False):
...
## Do the top-candidate and WZA on uncorrected data
		...
		if bayPass != "none":
			results = pd.concat([ distance, wza_results_corr, wza_results_kendall, wza_results_bayPass, TC_results_baypass, TC_results_kendall, SNP_results_bayPass, SNP_results_kendall, LA, position ] , axis = 1, sort = True ).reset_index()
			print(results)
		else:
			results = pd.concat([ distance, wza_results, wza_results_corr, TC_results, SNP_results, LA, position ] , axis = 1, sort = True ).reset_index()
...
## Make a list for each of the simulation parameter sets
	...
	for q in analyseSimSet(simSet, bayPass = args.bayPass, IBA = args.IBA, shuffle = args.shuffle, unweighted = args.unweighted):
